...
parser.add_argument(
    "--filename",
    help="enter the csv filename, file should be in the \
...
def createTable(tablename, columns):
    ...
    for col in columns:
        column_query += col + " VARCHAR(200), "
        # how to make this VARCHAR() length flexible
...
if __name__ == "__main__":
    ...
    query = f"INSERT INTO {args.filename[:-4]} ({', '.join(columns)}) VALUES \
        ({', '.join(str('%s '*len(columns)).split())})"
    ...
    print(f"{cursor.rowcount} rows were inserted in \
        the table: {args.filename[:-4]}")
    print('Done!')
